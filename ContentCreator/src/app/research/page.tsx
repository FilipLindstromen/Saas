...
export default function ResearchPage() {
  ...
  return <div className="space-y-4"><div className="flex items-center justify-between"><h2 className="text-2xl font-semibold">Research</h2><V1Badge /></div>
    <Card title="Research Inputs" subtitle="Instagram/YouTube/etc are selectable but not live-connected in v1.">
      ...
      <p className="cc-muted mt-3 text-xs">Sources</p><div className="mt-1 flex flex-wrap gap-2">{RESEARCH_SOURCES.map((s)=><button type="button" key={s} className="cc-chip" data-active={selectedSources.includes(s) ? "true" : "false"} onClick={()=>setSelectedSources((p)=>p.includes(s)?p.filter((x)=>x!==s):[...p,s])}>{s}</button>)}</div>
      <p className="cc-muted mt-3 text-xs">Platforms</p><div className="mt-1 flex flex-wrap gap-2">{platforms.map((p)=><button type="button" key={p} className="cc-chip" data-active={selectedPlatforms.includes(p) ? "true" : "false"} onClick={()=>setSelectedPlatforms((v)=>v.includes(p)?v.filter((x)=>x!==p):[...v,p])}>{p}</button>)}</div>
      <button type="button" className="cc-btn-primary mt-4" onClick={()=>setResults(generateResearchIdeas({topic,audience,platforms:selectedPlatforms,selectedSources,trendNotes,count},data.brandProfile))}>Generate opportunities</button>
    </Card>
    <Card title={`Results (${results.length})`} subtitle="Save selected ideas into pipeline.">{results.length===0?<p className="cc-muted text-sm">No results yet.</p>:<div className="space-y-2">{results.map((r)=><div key={r.id} className="cc-panel text-sm"><p className="font-medium">{r.title}</p><p className="cc-subtle">{r.whyTrending}</p><p className="cc-muted text-xs">Source: {r.selectedSource} | Scores: A{r.audienceSizeScore}/D{r.demoAbilityScore}/H{r.hookPotentialScore}</p><button type="button" className="cc-btn-secondary mt-2" onClick={()=>addIdeas([r])}>Save to pipeline</button></div>)}</div>}</Card>
  </div>
}
