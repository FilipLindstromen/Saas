...
1. Headlines (title) and descriptions (content) MUST match the item kind:
   ...
   b) Actionable and informational items (task, shopping, calendar, plus notes and ideas that are NOT primarily emotional): remove meta / assistant-command filler. Do NOT paste the user's full instruction sentence into content (e.g. avoid reproducing "I want to … so create a calendar event…").
      ...
      Also strip standalone temporal prefixes ("tomorrow", "today", "tonight", "next Monday", etc.) from **title** and **content** once the date is already captured in task_due_date, shopping_due_date, or scheduled_date. The date field is the authoritative source — do not repeat it in the text.
      ...
      • Reference examples (use reference date/time below for "today"/"tomorrow"):
        ...
        – "Tomorrow I need to buy groceries, climb a mountain, and do homework" → 3 separate items, ALL with task_due_date (or shopping_due_date for buying) = tomorrow. Titles: "Buy groceries", "Climb a mountain", "Do homework" — no "tomorrow" in any title or content since the date is already set.
      **Shared temporal context:** When one sentence or clause sets a date/time anchor ("tomorrow I need to …", "today: X, Y and Z", "by Friday do A, B, C") followed by multiple items, every item in that list inherits the **same** date. Set the correct date field on ALL items — not just the first. Never repeat the time word in title or content.
...
1. Rubrik (title) och beskrivning (content) ska spegla postens typ:
   ...
   b) Handlingsposter och fakta (task, shopping, calendar, samt anteckningar/idéer som INTE främst är känslomässiga): ta bort utfyllnadsfraser och "skapa en kalenderhändelse"-meta. Kopiera INTE hela instruktionsmeningen in i content (t.ex. inte "jag vill … så skapa ett kalenderevent …").
      ...
      Ta också bort fristående tidsprefix ("imorgon", "idag", "i kväll", "nästa måndag" osv.) från **title** och **content** när datumet redan är satt i task_due_date, shopping_due_date eller scheduled_date. Datumfältet är källan — upprepa det inte i texten.
      ...
      • Exempel (med referensdatum nedan för "idag"/"imorgon"):
        ...
        – "I morgon behöver jag köpa mat, klättra i berg och läsa läxor" → 3 separata poster, ALLA med rätt datum (shopping_due_date = imorgon för mat, task_due_date = imorgon för de andra). Titlar: "Köpa mat", "Klättra i berg", "Läsa läxor" — ingen skriver "imorgon" i title eller content eftersom datumet redan är satt.
      **Delat tidssammanhang:** När en mening eller sats sätter ett datum/tidsankare ("imorgon måste jag …", "idag: X, Y och Z", "senast fredag gör A, B, C") följt av flera poster, ärver **varje post** i listan **samma datum**. Sätt rätt datumfält på ALLA poster — inte bara den första. Upprepa aldrig tidsordet i title eller content.
