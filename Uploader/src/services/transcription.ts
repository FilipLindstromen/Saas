 ...
 * Uses the shared saas API key (saas_api_keys -> openai) from localStorage,
 * or a key passed directly.
 */
...
export async function transcribeVideo(videoBlob: Blob, apiKey: string): Promise<CaptionSegment[]> {
  const key = apiKey?.trim()
  if (!key) throw new Error('OpenAI API key is not set. Add it via Settings on the Saas Apps screen.')
  ...
  return (data.segments || []).map((s) => {
    const words = s.words
      ?.map((w) => ({ word: (w.word ?? w.text ?? '').trim(), start: w.start, end: w.end }))
