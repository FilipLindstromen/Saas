...
const UI_PORT = Number(process.env.NATIVE_ADS_UI_PORT) || 5177
const DEV_URL = process.env.NATIVE_ADS_DEV_URL || 'http://127.0.0.1:5176'
...
app.on('activate', () => {
  if (BrowserWindow.getAllWindows().length === 0) {
    boot().catch((error) => {
      console.error(error)
      dialog.showErrorBox('Native Ads Generator', error.message || 'Failed to start app')
    })
  }
})
...
process.on('uncaughtException', (error) => {
  ...
  dialog.showErrorBox('Native Ads Generator', error.message || 'An unexpected error occurred')
})
