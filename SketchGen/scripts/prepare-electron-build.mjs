...
function killSketchGenProcesses() {
  if (process.platform !== 'win32') return
  const names = ['SketchGen', 'electron']
  ...
  if (!tryRemoveWinUnpacked()) {
    ...
    console.warn(
      '[prepare-electron-build] Could not clear release/win-unpacked (files are locked).\n' +
        'Close SketchGen if it is running. Building to release-staging instead.'
    )
  } else {
