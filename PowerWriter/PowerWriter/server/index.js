...
app.post("/api/document/transcribe", async (req, res) => {
  try {
    ...
    // Process words to attach punctuation (especially periods) to the appropriate words
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      let wordWithPunctuation = wordText;
      
      // Check if next word is just punctuation - if so, merge it
      if (i < words.length - 1) {
        const nextWord = words[i + 1];
        const nextWordText = nextWord.word.trim();
        
        // If next word is just punctuation, attach it to current word
        if (/^[.,!?;:]+$/.test(nextWordText)) {
          wordWithPunctuation = wordText + nextWordText;
          skipIndices.add(i + 1);
        } else {
          // Check if there's punctuation between this word and the next in the full text
          // Find the word in the full text and check what comes after it
          const searchStart = i > 0 ? processedWords.length > 0 ? fullText.indexOf(processedWords[processedWords.length - 1].word) + processedWords[processedWords.length - 1].word.length : 0 : 0;
          const wordIndex = fullText.toLowerCase().indexOf(wordText.toLowerCase(), searchStart);
          if (wordIndex >= 0) {
            const afterWord = fullText.substring(wordIndex + wordText.length);
            const nextWordIndex = afterWord.toLowerCase().indexOf(nextWordText.toLowerCase());
            if (nextWordIndex > 0) {
              // There's text between the words - check if it's punctuation
              const between = afterWord.substring(0, nextWordIndex).trim();
              if (/^[.,!?;:]+$/.test(between)) {
                wordWithPunctuation = wordText + between;
              }
            }
          }
        }
      } else {
        // Last word - check if there's trailing punctuation in the full text
        const wordIndex = fullText.toLowerCase().lastIndexOf(wordText.toLowerCase());
        if (wordIndex >= 0) {
          const afterWord = fullText.substring(wordIndex + wordText.length).trim();
          // If what follows is just punctuation, attach it
          if (/^[.,!?;:]+$/.test(afterWord)) {
            wordWithPunctuation = wordText + afterWord;
          }
        }
      }
