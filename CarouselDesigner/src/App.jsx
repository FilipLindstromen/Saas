...
function App() {
  ...
  const loadSavedData = () => {
    try {
      const savedSlides = localStorage.getItem('carouselDesignerSlides')
      const savedSelectedId = localStorage.getItem('carouselDesignerSelectedId')
  ...
  const [chapters, setChapters] = useState(() => {
    const saved = localStorage.getItem('carouselDesignerChapters')
    if (saved) {
      try {
  ...
  const [currentChapterId, setCurrentChapterId] = useState(() => {
    const saved = localStorage.getItem('carouselDesignerCurrentChapterId')
  ...
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => localStorage.getItem('carouselDesignerSidebarCollapsed') === 'true')
  const [inspectorDrawerOpen, setInspectorDrawerOpen] = useState(false)
  const previousModeRef = useRef('concept')
  ...
  const [workspaces, setWorkspaces] = useState(() => {
    try {
      const saved = localStorage.getItem('carouselDesignerWorkspaces')
  ...
  const [currentWorkspace, setCurrentWorkspace] = useState(() => {
    return localStorage.getItem('carouselDesignerCurrentWorkspace') || 'default'
  })
  ...
  const [inspectorWidth, setInspectorWidth] = useState(() => {
    const saved = localStorage.getItem('carouselDesignerInspectorWidth')
    return saved ? parseInt(saved, 10) : 320
  })
  const [projectName, setProjectName] = useState(() => {
    return localStorage.getItem('carouselDesignerProjectName') || ''
  })
  ...
  const [recordSettings, setRecordSettings] = useState(() => {
    const saved = localStorage.getItem('carouselDesignerRecordSettings')
    if (saved) {
      try {
        const parsed = JSON.parse(saved)
        return {
  ...
  useEffect(() => {
    setChapters(prevChapters => {
      ...
      try {
        localStorage.setItem('carouselDesignerChapters', JSON.stringify(updated))
      } catch (error) {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem('carouselDesignerCurrentChapterId', currentChapterId.toString())
    } catch (error) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('carouselDesignerChapters', JSON.stringify(chapters))
    } catch (error) {
      console.error('Error saving chapters:', error)
    }
  }, [chapters])
  ...
  useEffect(() => {
    try {
      localStorage.setItem('carouselDesignerSelectedId', selectedSlideId.toString())
    } catch (error) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('carouselDesignerInspectorWidth', inspectorWidth.toString())
    } catch (error) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('carouselDesignerRecordSettings', JSON.stringify(recordSettings))
    } catch (error) {
  ...
  useEffect(() => {
    if (currentWorkspace) {
      ...
      localStorage.setItem(`carouselDesignerWorkspace_${currentWorkspace}`, JSON.stringify(workspaceData))
    }
  ...
  // Save recent files
  useEffect(() => {
    localStorage.setItem('carouselDesignerRecentFiles', JSON.stringify(recentFiles))
  }, [recentFiles])

  // Save workspaces
  useEffect(() => {
    localStorage.setItem('carouselDesignerWorkspaces', JSON.stringify(workspaces))
  }, [workspaces])
  ...
  const handleExportSlidesAsPng = useCallback(async () => {
    ...
    try {
      ...
      await exportSlidesAsPng({
        slides: allSlides,
        settings,
        slideFormat: settings.slideFormat || CAROUSEL_FORMAT,
        projectName,
  ...
  const handleApplyConceptToPlan = useCallback((ideas) => {
    ...
    setSlides(newSlides)
    setSelectedSlideId(newSlides[0]?.id ?? 1)
  ...
  const handleMetaCarouselExport = useCallback(async ({ adSetId, pageId, destinationUrl, primaryText, adName }) => {
    if (isExportingPng) return
    const allSlides = chapters.flatMap((ch) => ch.slides).filter((s) => (s.layout || 'default') !== 'section')
    if (!allSlides.length) {
      setMetaExportError('No slides to export.')
      return
    }

    setIsExportingPng(true)
    setMetaExportError('')
    setExportProgress('Rendering carousel images…')
    try {
      const { renderSlidesAsImageBlobs } = await import('./utils/exportSlidesAsPng.jsx')
      const blobs = await renderSlidesAsImageBlobs({
        slides: allSlides,
      ...
      if (!blobs.length) throw new Error('Could not render carousel images')

      setExportProgress('Uploading to Meta…')
      const result = await exportCarouselToMeta({
        accessToken: getApiKey('metaAccessToken'),
        adAccountId: getApiKey('metaAdAccountId'),
        pageId,
        adSetId,
        destinationUrl,
        imageBlobs: blobs,
  ...
  const handleSwitchWorkspace = (workspaceId) => {
    ...
    localStorage.setItem('carouselDesignerCurrentWorkspace', workspaceId)
    const workspaceData = localStorage.getItem(`carouselDesignerWorkspace_${workspaceId}`)
  ...
  const handleAddWorkspace = () => {
    ...
    setWorkspaces(next)
    localStorage.setItem('carouselDesignerWorkspaces', JSON.stringify(next))
  }
  ...
  useEffect(() => {
    try {
      localStorage.setItem('carouselDesignerSidebarCollapsed', String(sidebarCollapsed))
    } catch (_) {}
  }, [sidebarCollapsed])
  ...
  const handleNewPresentation = (projectNameFromModal) => {
    const doCreate = () => {
      ...
      localStorage.removeItem('carouselDesignerChapters')
      localStorage.removeItem('carouselDesignerCurrentChapterId')
    }
  ...
  useEffect(() => {
    localStorage.setItem('carouselDesignerProjectName', projectName)
  }, [projectName])
  ...
  // Present / recording removed — carousel designer focuses on static export
  const handleCommandPaletteAction = (action, arg) => {
    switch (action) {
      ...
      case 'exportMeta': setShowMetaExport(true); break
  ...
  return (
    <div className={`app${mode === 'concept' ? ' concept-mode-app' : ''}${mode === 'plan' ? ' plan-mode-app' : ''}${mode === 'preview' ? ' preview-mode-app' : ''}${mode === 'edit' ? ' edit-mode-app' : ''}`}>
      ...
      {showMetaExport && (
        <MetaCarouselExportModal
          isOpen={showMetaExport}
          onClose={() => { if (!isExportingPng) setShowMetaExport(false) }}
          onSubmit={handleMetaCarouselExport}
          busy={isExportingPng}
          error={metaExportError}
          slideCount={chapters.flatMap((ch) => ch.slides).filter((s) => (s.layout || 'default') !== 'section').length}
          defaultAdName={projectName || 'Carousel ad'}
          defaultPrimaryText={chapters.flatMap((ch) => ch.slides).map((s) => s.content).filter(Boolean).join('\n\n').slice(0, 500)}
        />
      )}
      {metaExportSuccess && (
        <div className="auto-save-indicator saved" role="status">
          <span>Carousel uploaded to Meta (ad {metaExportSuccess.adId || 'created'})</span>
          <button type="button" className="meta-export-success-dismiss" onClick={() => setMetaExportSuccess(null)}>Dismiss</button>
        </div>
      )}
      <input
        ref={fileInputRef}
