...
export async function renderSlidesAsImageBlobs({
  slides,
  settings,
  slideFormat = '3:4',
  onProgress,
  ...
  if (!exportSlides.length) throw new Error('No slides to export')
  ...
  try {
    ...
    for (let i = 0; i < exportSlides.length; i++) {
      ...
      onProgress?.(`Rendering slide ${i + 1} of ${exportSlides.length}…`)
      ...
      root.render(
        ...
        >
          <Slide slide={slide} {...slideProps} slideFormat={slideFormat} />
        </div>
      )
